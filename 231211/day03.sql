...
-- in
-- 부서번호가 10,20인 사원 이름, 부서번호 조회
select ename, deptno from emp where deptno = 10 or deptno = 20
-- 부서번호가 10,20, 30인 사원 이름, 부서번호 조회
...
-- where 절에는 집계함수를 사용할 수 없다. -> (having)
-- having은 group by 뒤에 오고, 그룹화된 결과 집합에서 조건을 적용할 때 사용한다.(실무에서 안씀)
select job, sum(sal) from emp group by job having sum(sal) >= 5000

-- 직업이 매니저이고, 직책별로 그룹핑 후 그룹핑된 결과 평균 급여 200이상인 사원 직책, 평균 급여 조회하시오.
-- where은 무조건 from 뒤에 
select job, avg(sal) from emp where job = 'MANAGER' group by job 
having avg(sal) >= 200 order by avg(sal) desc
-- SQL 실행순서 최종 정리
-- 1. from(무조건 첫번째) 2. where 3. group by 4. having 5. select 6. order by(무조건 마지막)
-- where와 having 차이점 다시 정리!
-- 1. where에는 sum,max,min,count,avg 집계함수 사용불가
-- 2. having은 group by 뒤에 오고, where은 from 뒤에 온다.
-- 3. **where 은 특정 조건을 만족하는 행을 필터링할 때, having은 그룹화된 결과에 조건을 적용할 때
...
-- where과 having은 목적이 다르기에 그룹화된 결과를 필터링할 때만 having을 쓰자
-- **group by는 그룹핑뿐만 아니라 정렬작업도 같이 이루어지기 때문에 데이터가 많을 땐 신중하게 사용!
-- distinct와 group by 차이는 정렬 작업이 이루어지냐 아니냐 차이
...
where date_format(hiredate, '%m') >= '02' and date_format(hiredate, '%m') <= '06' group by hiredate

-- 8. 부서별 직책별 최대급여와 인원수, 최소급여 조회 단, 10번 부서제외하고, 인원수는 3명 이하, 인원수가 적은 순서대로 정렬하시오. ?
select max(sal), count(*), min(sal) from emp
where deptno != 10 group by job, deptno having count(*) <= 3 order by count(*) asc

-- 9. SQL 실행순서로 올바른 것은? (2) 
...
-- 10. 다음 보기 중 세번째 문자가 'N'인 문자열을 검색하는 조건으로 적절한 것은? (4)
